...
- **Removed** from original: blob/avatar fields, FileForm subform, TagForm, timezone/zone-id field, `fo/query-inclusion`
  for blob status keys, semantic-ui rendering options (`suo/*`)
...
- Forms use `form/form-route-state` with `:route/params` (calls `form/start-form!` → `scf/start!`)
- Reports use `report/report-route-state` (calls `report/start-report!`)
- Landing page uses plain `scr/rstate` with backtick-quoted symbol
- Navigation uses `scr/route-to!` and `form/create!` for form creation
- No `sfr/` (rad_integration) dependency — routing helpers in `form.cljc` and `report.cljc`
