...
When a component is used as a route target via `istate`, the statecharts invocation system manages the child session automatically. The `child-session-id` option on `istate` can override this if needed. RAD does not need to manage this directly -- `istate` handles the lifecycle.

However, RAD public API functions (`save!`, `cancel!`, etc.) still need to discover the session-id. For routed forms, the session-id is the one created by `istate`'s invoke. The `send-to-self!` pattern from the routing library is the correct way to send events from within a routed component:

```clojure
;; Inside a routed form, the component can use:
(scf/send-to-self! this :event/save)
```

If the public API needs to send from outside (e.g. `form/save! app form-ident`), it can use `ident->session-id` as a fallback, but the primary pattern for routed forms is `send-to-self!`.

**Open question**: Does `istate` use the same deterministic session-id as `ident->session-id`, or does it generate its own? If `istate` uses a different session-id, then `form-session-id` cannot be used to send events to routed forms from the outside. This needs verification. The `child-session-id` option on `istate` could be set to `(ident->session-id ident)` to ensure consistency.

## Summary Table

| Module      | Session ID Source                         | Type    | Stable? | Example                                         |
|-------------|-------------------------------------------|---------|---------|--------------------------------------------------|
| Form        | `(ident->session-id form-ident)`          | Keyword | Yes     | `:com.fulcrologic.rad.sc/account_id--abc-123`    |
| Report      | `(ident->session-id report-ident)`        | Keyword | Yes     | `:com.fulcrologic.rad.sc/report_id--myapp...`    |
| Container   | `(ident->session-id container-ident)`     | Keyword | Yes     | `:com.fulcrologic.rad.sc/container_id--myapp...` |
| Auth        | `::auth-session`                          | Keyword | Yes     | `:com.fulcrologic.rad.sc/auth-session`           |
| Hooks       | `(hooks/use-gc-id)`                       | UUID    | Per-mount | `#uuid "..."` (random)                         |
| Routing     | Managed by `istate` invoke                | UUID    | Per-route | Auto-generated by invocation system            |
...
1. **Should `istate` use `ident->session-id` for its `child-session-id`?** This would make the routing-managed session discoverable by the RAD public API. The alternative is that routed forms always use `send-to-self!` and never need external session-id discovery.
